...
export default function Resources() {
    ...
    api.entities.StudyMaterial.list('-date', 200)
      .then(async (r) => {
        ...
        const enriched = await Promise.all(rows.map(async (item) => {
          ...
          if (item.thumbnail_url || !isBilibiliVideo) return item;
          try {
            const metadata = await api.bilibili.preview(item.file_url);
            return { ...item, thumbnail_url: metadata.thumbnail_url };
          } catch {
            return item;
          }
        }));
  ...
  const bilibiliItems = filtered.filter((item) => item.file_type === 'video' && hasBilibiliVideoId(item.file_url));
  const regularItems = filtered.filter((item) => item.file_type !== 'video' || !hasBilibiliVideoId(item.file_url));
          ...
          {filtered.length === 0 ? (
            ...
            <>
              ...
              {regularItems.length > 0 && (
                <m.div layout={!reduceMotion} className="mt-8 max-w-5xl divide-y divide-border/75 border-y border-border/75">
                  <AnimatePresence mode="popLayout" initial={false}>
                    {regularItems.map((item) => {
                      const t = TYPE_ICON[item.file_type] || TYPE_ICON.other;
                      const isDirectVideo = item.file_type === 'video' && item.file_url;
                      const rowContent = (
                        <>
