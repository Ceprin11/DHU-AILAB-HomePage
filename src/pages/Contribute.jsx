        ...
        {type === 'award' && <div className="mt-10">
          <form onSubmit={(event) => { event.preventDefault(); submit('Award', award, () => setAward(emptyAward()), '成果投稿成功'); }} className="space-y-5 rounded-2xl border border-border bg-card p-5 sm:p-7">
            <div className="grid gap-5 sm:grid-cols-2">
              ...
              </div> : <div><Label htmlFor="award-ccf">CCF 等级 / 中科院几区</Label><Input id="award-ccf" maxLength={100} value={award.ccf_level} onChange={(event) => setAward({ ...award, ccf_level: event.target.value })} className={fieldClass} placeholder="如：CCF-A、中科院一区" /></div>}
